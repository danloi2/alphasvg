...
def generate_grayscale_svg(
    ...
    try:
        ...
        print(f"🎨 SVG Grayscale OK: {os.path.basename(output_path)} ({len(svg_layers)} tonos)")
        ...
        for y in range(0, height, spacing):
            for x in range(0, width, spacing):
                if x < width and y < height:
                    # Obtener valor de gris en esta posición
                    gray_val = gray_array[y, x]
                    
                    # Calcular radio del círculo basado en oscuridad
                    # Más oscuro = círculo más grande
                    darkness = 1 - (gray_val / 255.0)
                    radius = (dot_size * darkness) * 0.8
                    
                    if radius > 0.5:  # Solo dibujar si es visible
                        circles.append(f'<circle cx="{x}" cy="{y}" r="{radius:.2f}" fill="#000" />')
        
        # Guardar SVG
        with open(output_path, "w") as f:
            f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
            f.write(f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">\n')
            f.write('  <desc>Halftone effect</desc>\n')
            f.write('  <rect width="100%" height="100%" fill="white"/>\n')
            for circle in circles:
                f.write(f'  {circle}\n')
            f.write('</svg>\n')
        
        print(f"🎨 SVG Halftone OK: {os.path.basename(output_path)} ({len(circles)} puntos)")
...
def generate_lineart_svg(
    ...
    """
    Genera SVG de alto contraste (estilo dibujo/silueta) usando Potrace.
    """
    if os.path.exists(output_path):
        return
    ...
    try:
        ...
        subprocess.run([
            "potrace", temp_bmp, "-s", "-o", output_path, 
            "--flat", 
            "--turdsize", str(turdsize), 
            "--alphamax", str(alphamax), 
            "--opttolerance", "0.2"
