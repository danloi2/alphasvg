...
def generate_color_svg(
    ...
    try:
        ...
        if is_bw:
            ...
            mask_arr = np.where((gray < threshold) & visible_mask, 0, 255).astype(np.uint8)
            ...
            subprocess.run([
                "potrace", t_bmp, "-s", "-o", t_svg,
                "--flat", 
                "--turdsize", str(turdsize),
                "--alphamax", "0.5"  # Suavizar curvas
            ], check=True, capture_output=True)
            
            with open(t_svg, "r") as f:
                content = f.read()
            ...
            if path_match:
                ...
                svg_layers.append(f'<g transform="{transform}"><path d="{d}" fill="#000000" stroke="none" /></g>')
            ...
            kmeans = KMeans(n_clusters=min(num_colors, len(visible_pixels)), 
            ...
            for idx in sorted_indices:
                ...
                subprocess.run([
                    ...
                    "--flat",
                    "--turdsize", str(turdsize),
                    "--alphamax", "0.8",  # Curvas más suaves para color
                    "--opttolerance", "0.2"
                ], check=True, capture_output=True)
                ...
                if path_match:
                    ...
                    svg_layers.append(f'<g transform="{transform}"><path d="{d}" fill="{hex_color}" stroke="none" /></g>')
        ...
        with open(output_path, "w") as f:
            ...
            f.write(f'<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">\n')
            for layer in svg_layers:
                f.write(f'  {layer}\n')
            f.write('</svg>\n')
        
        print(f"🎨 SVG generado: {os.path.basename(output_path)} ({len(svg_layers)} capas)")
...
if __name__ == "__main__":
    ...
    generate_color_svg("illustration.png", "illustration.svg", num_colors=48, blur_radius=1)