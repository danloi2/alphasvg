...
class ImageProcessorGUI:
    ...
    def _setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Procesador Transparente", font=("Helvetica", 16, "bold"))
        title_label.pack(pady=(0, 20))

        # Input Selection Group
        input_group = ttk.LabelFrame(main_frame, text=" 1. Imagen de entrada (.png, .jpg) ", padding="10")
        input_group.pack(fill=tk.X, pady=5)
        
        ttk.Entry(input_group, textvariable=self.input_file).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        btn_in = ttk.Button(input_group, text="Buscar Archivo", command=self._browse_input)
        btn_in.pack(side=tk.RIGHT)

        # Output Selection Group
        output_group = ttk.LabelFrame(main_frame, text=" 2. Carpeta de destino (donde guardar) ", padding="10")
        output_group.pack(fill=tk.X, pady=5)
        
        ttk.Entry(output_group, textvariable=self.output_dir).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        btn_out = ttk.Button(output_group, text="Elegir Carpeta", command=self._browse_output)
        btn_out.pack(side=tk.RIGHT)

        # Process Button
        self.process_btn = ttk.Button(main_frame, text="INICIAR CONVERSION", command=self._process_image)
        self.process_btn.pack(pady=30)

        # Status Bar
        status_frame = ttk.Frame(main_frame)
        status_frame.pack(fill=tk.X, side=tk.BOTTOM)
        ttk.Label(status_frame, textvariable=self.status_var, font=("Helvetica", 9, "italic")).pack(side=tk.LEFT)
    ...
    def _process_image(self):
        ...
        print(f"DEBUG: [GUI] Iniciar Conversión pulsado. Input: {input_path}, Output: {output_dir}")
        ...
        if not shutil.which("potrace"):
            messagebox.showerror("Error", "No se encontró 'potrace' en el sistema.\nPor favor, instálalo antes de continuar.")
            return
        ...
        def run_thread():
            try:
                ...
                paths = {
                    ...
                    "color_logo": os.path.join(output_dir, base_name + "_color_logo.svg"),
                    "color_illus": os.path.join(output_dir, base_name + "_color_illus.svg"),
                ...
                if not os.path.exists(alpha_processed):
                    raise Exception("Fallo en la generación del PNG Alpha por IA (archivo no creado).")
                ...
                self.root.update_idletasks()
                generators.generate_color_svg(alpha_processed, paths["color_logo"], num_colors=16, blur_radius=0.5)

                print("DEBUG: [THREAD] Generando SVG Color (Ilustración)...")
                self.status_var.set("Generando SVG Color (Ilustración)...")
                self.root.update_idletasks()
                generators.generate_color_svg(alpha_processed, paths["color_illus"], num_colors=48, blur_radius=1)
