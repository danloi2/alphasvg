...
def main():
    parser = argparse.ArgumentParser(description="Procesador de imágenes por lotes (CLI)")
    parser.add_argument("--input", "-i", help="Carpeta con las imágenes originales", required=True)
    parser.add_argument("--output", "-o", help="Carpeta donde se guardarán los resultados", required=True)
    ...
    files = [f for f in os.listdir(input_dir) 
             if f.lower().endswith(extensions) and ".temp." not in f and ".vtrace_temp." not in f]
    ...
    for file in files:
        ...
        if os.path.exists(alpha_path):
            ...
            generators.generate_color_svg(alpha_path, color_logo_path, num_colors=16, blur_radius=0.5)
            generators.generate_color_svg(alpha_path, color_illus_path, num_colors=48, blur_radius=1)
            generators.generate_thumbnail(alpha_path, thumb_path)
        else:
            print(f"⚠️ Skipping vectorization for {file} because Alpha PNG was not created.")
